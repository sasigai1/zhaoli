...
// Hermetic cwd: an empty workspace with no src/lib/og/site.json and no
// public/og.jpg, so fixtures (appName / document title / site) are the only
// inputs. A real app workspace brands those files, which would otherwise
// override every title/card expectation below.
const hermeticRoot = () => mkdtempSync(join(tmpdir(), "grok-pwa-hermetic-"));
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd: hermeticRoot() });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", cwd: hermeticRoot() });
  const chunks = [
