...
function applyTheme(t) {
  ...
  // Android WebView 桥接: 同步状态栏颜色
  if (window.MojianNative) { try { window.MojianNative.setDarkMode(t === 'dark'); } catch (e) { /* 非安卓环境 */ } }
}

/* Android 返回键处理: 返回 true 表示已消费(关抽屉/退出编辑/回首页), false 交给系统 */
window.__mojianBack = function () {
  if (document.body.classList.contains('drawer-open')) { closeDrawer(); return true; }
  if (state.editingId) { state.editingId = null; render(); return true; }
  if (state.view !== 'memo') { go('memo'); return true; }
  return false;
};
